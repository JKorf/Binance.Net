...
namespace Binance.Net
{
    ...
    /// Rate limiter configuration for the Binance API
    /// </summary>
    public class BinanceRateLimiters : IAccessLimiter
    {
        /// <summary>
...
#pragma warning restore CS8618 // Non-nullable field must contain a non-null value when exiting constructor. Consider declaring as nullable.
        ...
        private void Initialize()
        {
            SpotRestIp = new RateLimitGate("Spot Rest")
                                            ...
                                            .AddGuard(new RateLimitGuard((def, host, key) => def.Path + def.Method, new PathStartFilter("sapi/"), 12000, TimeSpan.FromMinutes(1), RateLimitWindowType.Fixed)); // IP limit of 12000 request weight per endpoint per minute to /sapi endpoints
            SpotRestUid = new RateLimitGate("Spot Rest")
                                            ...
                                            .AddGuard(new RateLimitGuard((def, host, key) => key?.GetString() + def.Path + def.Method, new PathStartFilter("sapi/"), 180000, TimeSpan.FromMinutes(1), RateLimitWindowType.Fixed)); // Uid limit of 180000 request weight per minute to /sapi endpoints
            SpotSocket = new RateLimitGate("Spot Socket")
                                            ...
                                            .AddGuard(new RateLimitGuard((def, host, key) => host, new IGuardFilter[] { new HostFilter("wss://ws-api.binance.com") }, 6000, TimeSpan.FromMinutes(1), RateLimitWindowType.Fixed, connectionWeight: 2)); // 6000 request weight per minute in total
