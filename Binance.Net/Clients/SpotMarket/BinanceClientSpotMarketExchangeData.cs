...
namespace Binance.Net.Clients.Rest.Spot
{
    ...
    /// </summary>
    public class BinanceClientSpotMarketExchangeData : IBinanceClientSpotMarketExchangeData
    {
        ...
        internal BinanceClientSpotMarketExchangeData(Log log, BinanceClientSpotMarket baseClient)
        {
            _log = log;
        ...
        public async Task<WebCallResult<DateTime>> GetServerTimeAsync(bool resetAutoTimestamp = false, CancellationToken ct = default)
        {
            ...
            else
            {
                ...
                if (BinanceClientBaseSpot.TimeSynced && !resetAutoTimestamp)
                ...
                if (offset >= 0 && offset < 500)
                {
                    ...
                    BinanceClientBaseSpot.CalculatedTimeOffset = 0;
                    BinanceClientBaseSpot.TimeSynced = true;
                    BinanceClientBaseSpot.LastTimeSync = DateTime.UtcNow;
                    _log.Write(LogLevel.Information, $"Time offset between 0 and 500ms ({offset}ms), no adjustment needed");
                    return result.As(result.Data.ServerTime);
                }

                BinanceClientBaseSpot.CalculatedTimeOffset = (result.Data.ServerTime - localTime).TotalMilliseconds;
                BinanceClientBaseSpot.TimeSynced = true;
                BinanceClientBaseSpot.LastTimeSync = DateTime.UtcNow;
                _log.Write(LogLevel.Information, $"Time offset set to {BinanceClientBaseSpot.CalculatedTimeOffset}ms");
                return result.As(result.Data.ServerTime);
            }
        }
        ...
        {
            var url = _baseClient.Options.OptionsSpot.BaseAddress.Replace("api.", "www.").AppendPath("exchange-api/v2/public/asset-service/product/get-products");
