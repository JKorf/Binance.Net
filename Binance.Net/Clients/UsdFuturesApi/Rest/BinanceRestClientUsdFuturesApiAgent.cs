...
namespace Binance.Net.Clients.UsdFuturesApi
{
    ...
    internal class BinanceRestClientUsdFuturesApiAgent : IBinanceRestClientUsdFuturesApiAgent
    {
        ...
        public async Task<HttpResult<BinanceFuturesIfNewUser>> GetIfNewUserAsync(string brokerId, int? receiveWindow = null, CancellationToken ct = default)
        {
            ...
            parameters.Add("type", IfNewUserMarginedFuturesType.UsdtMarginedFutures);
            parameters.AddOptionalParameter("recvWindow", receiveWindow?.ToString(CultureInfo.InvariantCulture) ?? _baseClient.ClientOptions.ReceiveWindow.TotalMilliseconds.ToString(CultureInfo.InvariantCulture));

            var request = _definitions.GetOrCreate(HttpMethod.Get, _baseClient.BaseAddress, "fapi/v1/apiReferral/ifNewUser", BinanceExchange.RateLimiter.FuturesRest, 100, true);
