...
namespace Binance.Net
{
    ...
    public class BinanceSocketClient : SocketClient, IBinanceSocketClient
    {
        ...
        /// <param name="onMessage">The event handler for the received data</param>
        /// <returns>A stream subscription. This stream subscription can be used to be notified when the socket is disconnected/reconnected</returns>
        public async Task<CallResult<UpdateSubscription>> SubscribeToSymbolMiniTickerUpdatesAsync(string symbol, Action<BinanceStreamMiniTick> onMessage)
        {
            symbol.ValidateBinanceSymbol();
            return await Subscribe(symbol.ToLower() + SymboMiniTickerStreamEndpoint, false, onMessage).ConfigureAwait(false);
        }
        ...
        /// <param name="onMessage">The event handler for the received data</param>
        /// <returns>A stream subscription. This stream subscription can be used to be notified when the socket is disconnected/reconnected</returns>
        public CallResult<UpdateSubscription> SubscribeToAllBookTickerUpdates(Action<IEnumerable<BinanceBookTick>> onMessage) => SubscribeToAllBookTickerUpdatesAsync(onMessage).Result;
        ...
        /// <param name="onMessage">The event handler for the received data</param>
        /// <returns>A stream subscription. This stream subscription can be used to be notified when the socket is disconnected/reconnected</returns>
        public async Task<CallResult<UpdateSubscription>> SubscribeToAllBookTickerUpdatesAsync(Action<IEnumerable<BinanceBookTick>> onMessage)
        {
        ...
        {
            if (data.ToString() == "ping frame")
                connection.Send("pong frame");
        }
